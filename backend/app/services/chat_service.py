...
TOOLS = [
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "appointment). Requires a real end_time reflecting a duration the user actually "
                "stated or implied — never invent one; if they gave neither a duration nor an end "
                "time, ask them before calling this tool at all. Set is_blocking=true for "
                "essentially every real commitment — meetings, interviews, standups/DSUs, calls, "
                "appointments all occupy the calendar. Only use is_blocking=false for something "
                "explicitly informational that doesn't reserve the person's time."
            ),
            "parameters": {
                "type": "object",
                "properties": {
...
def _build_system_prompt(reference_datetime: datetime, timezone_name: str) -> str:
    return (
        ...
        "Before calling create_engagement — and before update_engagement when start_time/end_time "
        "is being changed — make sure you actually have what you need instead of filling a gap "
        "with a guess. If the user gave no explicit duration and no end time, ask how long it "
        "should be rather than assuming any particular length. If they named a bare day-of-week "
        "with no 'next'/'this' qualifier and nothing else in the conversation makes which "
        "occurrence obvious, ask which week they mean rather than silently taking the soonest one "
        "— e.g. 'this Wednesday or next Wednesday?'. If they explicitly named a timezone or zone "
        "abbreviation you can't confidently resolve to a specific IANA zone, ask them to clarify "
        "it rather than guessing or quietly falling back to their own zone. When more than one of "
        "these is missing, fold them into a single natural clarifying question instead of asking "
        "one at a time, then make the tool call once they've answered — don't call the tool with a "
        "placeholder now and fix it later. None of this applies to what you can already resolve "
        "yourself: an explicit date, 'today'/'tomorrow', or a timezone the user simply didn't "
        "mention at all, which correctly defaults to their own as described above — that default "
        "isn't a guess, so don't ask about it.\n\n"
